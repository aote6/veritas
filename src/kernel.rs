    ...
    pub fn handle(
        ...
        match call {
            ...
            KernelCall::MemoryAlloc {
                ...
                let state_id = self.engine.memory_alloc(ctx, object_id, size_hint)?;
                Ok(TrapResult::StateId(state_id))
            }
...
mod kernel_tests {
    ...
    fn memory_alloc_sequential_state_ids() {
        ...
        let sid0 = match kernel.handle(&mut ctx, call0).unwrap() {
        ...
        let sid1 = match kernel.handle(&mut ctx, call1).unwrap() {
            TrapResult::StateId(id) => id,
            other => panic!("expected StateId, got {:?}", other),
        };
