# Veritas World State Constitution v1.0

> 本文档已从设计文档升级为 Constitution。
> World State 是 Veritas Machine 在某一机器周期结束后的全部硬件状态。
> 所有 Checkpoint、Replay、Receipt、Recovery 必须以恢复或验证 World State 为目标。

## 宪法地位

本文档与 object.md、memory.md、transaction.md 同级。
Machine State 的定义是 Veritas 最核心的不变量。
