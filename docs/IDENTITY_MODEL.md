...
target == ctx.current_object || target == ctx.capability_context
→ 直接放行（自己人不用查）
已 commit 的 capability（ctx.capabilities）里，是否有一张
resource 匹配、且当前身份持有的 → 放行
本事务内 pending 的 grant（ctx.pending_capabilities）里，
是否有一条 grantee 匹配当前身份，或者 capability_id 已经
attach 到 ctx → 放行
capability graph 里，当前身份是否持有任意一张 resource 匹配
的、真正生效的 cap → 放行
都不满足 → 拒绝
...
capability_id 授予了某个 grantee"。这条记录要到 commit 时才
...
Machine dispatch 无 _ => {} 死代码
完成
编译期保证（non-exhaustive match）
OBJECT_BIRTH 不再隐式切身份
完成
object_birth_self_call.rs
OBJECT_BIRTH 自动 attach self-AdminCap
完成
object_birth_self_call.rs
CapabilityGrant{holder,resource} 用 Operand
完成
编译验证 + 全量测试绿
Call{object_id} 用 Operand
完成
call_access_intent.rs + 编译验证
已知未完成：
...
6. 改动这个子系统时的检查清单
改 Instruction 枚举字段类型前，先问：这个值可能来自
运行时（前序指令产生）吗？是 → 必须是 Operand，不能是裸值。
改完 instruction.rs，必须同步改 assembler.rs（解析）、
...
这个变体）。
任何涉及 current_object 赋值的改动，先搜索
grep -n "current_object" -r src/，确认改动前后这个值的
语义没有被破坏（谁读它、读出来干什么）。
任何"这个 case 反正安全可以跳过审计"的想法，先写测试验证
走正规路径（CALL + authorize_intent）是否真的可行，不要
凭直觉判断。
改完必须 cargo build 全部干净（含 warning 里的
non-exhaustive match 检查），然后 cargo test 2>&1 | grep -E "test result|FAILED" 确认全绿，两步都不能跳过。
涉及安全/权限的改动，必须补一个"恶意路径必须拒绝 +
