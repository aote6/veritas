...
fn world_demo_multi_object_birth_write_link_commit_recover() {
    ...
    world.tx_write(sid, 0, b"hello-A".to_vec(), Some(a)).expect("write A failed");
    ...
    world.tx_write(sid, 0, b"hello-B".to_vec(), Some(b)).expect("write B failed");

    let receipt = world.tx_commit(sid).expect("single commit failed");
    println!("demo commit: tx_id={} before_root={} after_root={}", receipt.tx_id, receipt.before_root, receipt.after_root);
    ...
    assert!(links.iter().any(|l| l.from == a && l.to == b), "A->B link should exist pre-restart");
    ...
    assert_eq!(obj_a2.state, ObjectState::Alive, "A must remain Alive after recovery");
    let obj_b2 = world2.get_object(b).expect("B should survive restart");
    assert_eq!(obj_b2.state, ObjectState::Alive, "B must remain Alive after recovery");

    let links2 = world2.list_links();
    assert!(links2.iter().any(|l| l.from == a && l.to == b), "A->B link should survive restart");
