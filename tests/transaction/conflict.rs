...
fn t2_conflict_detection() {
    ...
    let mut tx_setup = tk.kernel.begin_in_object(tk.root_object);
    tk.kernel.write(&mut tx_setup, state_id, initial_data).unwrap();
    tk.kernel.handle(&mut tx_setup, KernelCall::Commit).unwrap();

    // Tx1 reads
    let mut tx1 = tk.kernel.begin_in_object(tk.root_object);
    let _ = tk.kernel.read(&mut tx1, state_id);

    // Tx2 writes and commits
    let mut tx2 = tk.kernel.begin_in_object(tk.root_object);
    let tx2_data = vec![99];
    tk.kernel.write(&mut tx2, state_id, tx2_data).unwrap();
    ...
    let tx1_data = vec![1];
    tk.kernel.write(&mut tx1, state_id, tx1_data).unwrap();
